...
describe('getLandingCardFundingState', () => {
  it('shows managed Recoverable Grant progress against its summary deadline without AON status handling', () => {
  ...
  it('keeps expired managed Recoverable Grants out of AON failed and claim states', () => {
    const state = getLandingCardFundingState({
      fundingSummary: {
        endsAt: Date.now() - 1,
        percentageFunded: 50,
