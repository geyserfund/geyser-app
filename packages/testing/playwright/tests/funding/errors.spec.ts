/** Generic funding error handling through deterministic GraphQL interception */
...
import { ONCHAIN_AMOUNT, TEST_COMMENT, TEST_EMAIL } from '../../domains/funding/constants'
import { checkLiveBackendAvailability } from '../../domains/shared/backend'
import { ApolloErrors, ENV } from '../../domains/shared/constants'
  ...
  await page.route('**/graphql', async (route) => {
    ...
    if (requestBody?.operationName === 'ContributionCreate') {
      await route.fulfill({
        ...
        body: JSON.stringify({ data: null, errors: [{ extensions: extension }] }),
      })
      return
    }
...
test.describe('Funding Error Handling', () => {
  ...
  test.beforeEach(async ({ page }) => {
    test.skip(!ENV.FUNDING_PROJECT_NAME, 'Skipping funding tests: FUNDING_PROJECT_NAME is required.')
    const backend = await checkLiveBackendAvailability(page.request)
    test.skip(!backend.ok, `Skipping funding tests: ${backend.reason}`)
    await page.goto(`/project/${ENV.FUNDING_PROJECT_NAME}`)
  })

  test('shows wallet max limit error page', async ({ page }) => {
    await goToFundingCommentStepForDonation(page)
    await interceptContributionCreateWithError(page, { code: ApolloErrors.INVALID_FUNDING_AMOUNT, maxAmount: 50000 })
  ...
  test('shows wallet min limit error page', async ({ page }) => {
    await goToFundingCommentStepForDonation(page)
    await interceptContributionCreateWithError(page, { code: ApolloErrors.INVALID_FUNDING_AMOUNT, minAmount: 80000 })
    await submitFundingComment(page)
  ...
  test('shows wallet unreachable error page', async ({ page }) => {
    await goToFundingCommentStepForDonation(page)
    await interceptContributionCreateWithError(page, { code: ApolloErrors.WALLET_UNREACHABLE })
    await submitFundingComment(page)
  ...
  test('shows inactive project error page', async ({ page }) => {
    await goToFundingCommentStepForDonation(page)
    await interceptContributionCreateWithError(page, { code: ApolloErrors.NON_ACTIVE_PROJECT })
    await submitFundingComment(page)
  ...
  test('shows reward out of stock error page', async ({ page }) => {
    ...
    test.skip(
      ...
      'Skipping: FUNDING_PROJECT_NAME has no visible products/rewards to test the out-of-stock error.',
    )
    await clickToggleDonationInput(page)
    ...
    await interceptContributionCreateWithError(page, { code: ApolloErrors.REWARD_OUT_OF_STOCK })
    await clickContinueFromDetails(page)
  ...
  test('shows generic error page for internal server errors', async ({ page }) => {
    await goToFundingCommentStepForDonation(page)
    await interceptContributionCreateWithError(page, { code: ApolloErrors.INTERNAL_SERVER_ERROR })
    await submitFundingComment(page)
