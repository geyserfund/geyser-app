...
type TwitterWidgets = {
  createTweet?: (
    ...
    options: { width: string; theme: 'dark' | 'light' },
  ) => Promise<HTMLElement | null>
  load?: (element?: HTMLElement) => void
}

/** Safely reads the optional Twitter widgets global injected by third-party script. */
export const getTwitterWidgets = (): TwitterWidgets | undefined => {
  return (window as typeof window & { twttr?: { widgets?: TwitterWidgets } }).twttr?.widgets
}
...
export const createTweetEmbedMarkdown = async (url: string, isDarkMode: boolean): Promise<string | null> => {
  ...
  if (!validateTwitterUrl(url) || !twitterWidgets?.createTweet) {
    return null
  }
  ...
  try {
    const tweetElement = await twitterWidgets.createTweet(tweetId, container, {
      width: '350px',
