  ...
  const onSubmit = ({ rewardCurrency }: ProjectCurrencyVariables) => {
    if (project) {
      ...
      if(currentRewardCurrency == project?.rewardCurrency) {
        toast({
          title: 'failed to update project',
          description: `Your project is already set to ${currentRewardCurrency === RewardCurrency.Btcsat ? t('BTC (sats)') : t('USD ($)') } currency.`,
          status: 'error',
        })
      } else {
    ...
    >
      <VStack width="100%" alignItems="flex-start" spacing={6} flexGrow={1}>
        <FieldContainer title={t('Reward Type')}>
            ...
            <Select value={watch('rewardCurrency')} onChange={(event) => {
                setValue(
                    'rewardCurrency',
                    event.target.value as RewardCurrency
                  )
            }}>
