...
import { Box, Modal, ModalBody, ModalCloseButton, ModalContent, ModalFooter, ModalHeader, ModalOverlay, Text, Link } from '@chakra-ui/react';
...
export const QRModal = ({isOpen, onClose, images, projectId, title, setCopy, imageDownload}: IQRModal) => {
	const isMobile = isMobileMode();

	const lnurlPayUrl = encode(`${API_SERVICE_ENDPOINT}/lnurl/pay?projectId=${projectId}`);
				...
				<ModalFooter>
					<Box w="100%">
						{imageDownload.length === 0
							...
							: <Link w="100%" h="100%" _hover={{textDecoration: 'none'}} href={imageDownload} download={`${name}-lnaddress-qr.png`} isExternal>
