import { Text, Grid, Box, VStack, GridItem, UnorderedList, ListItem, Image, useMediaQuery } from '@chakra-ui/react';
...
export const Launch = () => {
	...
	>
		<Grid width="100%" templateColumns={isLargerThan1280 ? 'repeat(6, 1fr)' : isMobile ? 'repeat(2, 1fr)' : 'repeat(5, 1fr)' } padding={isMobile ? '10px' : '40px 40px 20px 40px'} >
			...
			<GridItem display="flex" colSpan={isLargerThan1280 ? 2 : isMobile ? 2 : 3} justifyContent="center" alignContent="center">
				<VStack
					spacing="30px"
					width="100%"
					maxWidth="350px"
					...
					marginBottom="40px"
				>
					<Text color="brand.gray500" fontSize="30px" fontWeight={700} paddingTop={isMobile ? 5 : 0} paddingBottom="20%"> Create a new Project</Text>
					<VStack width="100%" justifyContent="center">
						...
						<UnorderedList color={colors.neutral700} width="90%" fontSize="15px">
							...
							<ListItem color={colors.neutral500}>Gain subscribers (soon)</ListItem>
						</UnorderedList>
						{
							(!loading && (!user || (user && !user.id) || (user && !hasTwitterAccount(user))))
								? (
									<VStack>
										<Text color={colors.neutral700} paddingBottom={3}>We require creators to login with Twitter to create a project.</Text>
										<TwitterConnect />
									</VStack>
								)
