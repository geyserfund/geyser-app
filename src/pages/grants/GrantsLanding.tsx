				...
				<Box display="flex" justifyContent="center" alignItems="center" px={isMobile ? 0 : 20}>
					<Box overflow="auto" w={isMobile ? '95%' : '1226px'} display="flex" px={2}>
						{(!loading && grants.length > 0) && grantsSorted.map((grant: IProject) => {
							...
							return (<GrantCard key={grant.id} project={grant} number="1" date="JULY 1 - 31" distributed={grant.name === 'bitcoin-education' ? (0 / 1000000).toFixed(0) : grant.name === 'bitcoin-culture' ? (0 / 1000000).toFixed(0) : grant.name === 'bitcoin-development' ? (0 / 1000000).toFixed(0) : ''} status={'pending'} marginRight={grant.name !== 'bitcoin-culture' && true} />);
						})}
						{loading
							&& <>
								<Box>
