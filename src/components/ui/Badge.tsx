...
export const Badge = ({ badge }: { badge: string }) => {
	...
	return (
		<Box position="relative" as="span" backgroundColor="brand.bgGold" padding="0px 8px" borderRadius="20px" onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}>
			{hover
&& <>
	<Box position="absolute" top="-102px" left="-93px" p={2} zIndex={2} bg="#5B5B5B" rounded="lg">
		<Text color="white" fontWeight="bold">Emoji badges</Text>
		<HStack mt={1}>
			<VStack rounded="lg" bg="rgba(0, 0, 0, 0.11)" px={3}>
				<Box>
					🏅
				</Box>
	...
	<Box position="absolute" top="-25px" left="-5px" zIndex={2} borderLeft="20px solid transparent" borderRight="20px solid transparent" borderTop="20px solid #5B5B5B"/>
</>
			}
