...
import { useContributionEmailUpdateMutation } from '@/types/index.ts'
...
export const UpdateFundingContributionEmailAddress = () => {
  ...
  const [updateEmail, { loading }] = useContributionEmailUpdateMutation({
    onCompleted(data) {
      toast.success({
        title: t('Email updated'),
        description: t('Your email has been updated.'),
      })
      setUpdatedEmail(data.contributionEmailUpdate.email || '')
    },
    onError(error) {
      toast.error({
        title: t('Error'),
