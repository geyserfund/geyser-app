...
/** PayoutRsk: Component for handling claiming payouts with Lightning or On-Chain Bitcoin */
export const PayoutRsk: React.FC<PayoutRskProps> = ({ isOpen, onClose, satsAmount }) => {
  ...
  const handleLightningSubmit = async (data: LightningPayoutFormData) => {
    ...
    try {
      // TODO: Implement actual Lightning payout API call
      console.log('Lightning payout data:', data)
      ...
      setIsProcessed(true)
      toast.success({
        title: t('Claim initiated successfully'),
  ...
  const handleBitcoinSubmit = async (data: BitcoinPayoutFormData) => {
    setIsSubmitting(true)
    try {
      // TODO: Implement actual Bitcoin on-chain payout API call
      console.log('Bitcoin payout data:', data)
    ...
    return (
      <Modal isOpen={isOpen} onClose={handleClose} size="lg" title="">
        <VStack w="full" spacing={6} p={6}>
          ...
          {selectedMethod === PayoutMethod.Lightning ? (
            <LightningPayoutProcessed isRefund={false} onClose={handleClose} />
          ) : (
            <BitcoinPayoutProcessed isRefund={false} onClose={handleClose} />
          )}
        </VStack>
      </Modal>
    )
