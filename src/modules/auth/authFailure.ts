...
export const getAuthFailureMessage = (t: TFunction, code?: string, fallback?: string, provider?: string): string => {
  switch (code) {
    ...
    case 'OAUTH_EMAIL_ALREADY_EXISTS':
      return provider
        ? t('An account already exists for this email. Sign in with email, then connect {{provider}}.', { provider })
